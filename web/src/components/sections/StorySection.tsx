...
const monopoleSrc =
  process.env.NODE_ENV === "production" ? "/tesne/monopole_rotated.svg" : "/monopole_rotated.svg";
...
export const StorySection = () => {
  ...
  const TIMING = {
    sensorIn: 0.05,
    handlebarsIn: 0.25,
    handlebarsOutStart: 0.36,
    handlebarsOutEnd: 0.395,
    monopoleIn: 0.395,
    monopoleSettle: 0.417,
    monopoleHold: 0.47,
    monopoleOut: 0.495,
    beamOut: 0.5,
    ...
    carCenter: 0.8, // Auto je presne v strede (y = 0)
  ...
  // --- 1. Senzor ---
  const sensorOpacity = useTransform(
    scrollYProgress,
    [0.0, TIMING.sensorIn, TIMING.sceneFadeOut, 1.0],
    [0, 1, 1, 0]
  );
  const sensorScale = useTransform(
    scrollYProgress,
  ...
  const handlebarsX = useTransform(
    ...
    [
      0.2,
      TIMING.handlebarsIn,
      TIMING.handlebarsOutStart,
      TIMING.handlebarsOutEnd,
      TIMING.monopoleOut,
      TIMING.beamOut,
    ],
    ["100vw", "0vw", "0vw", "88vw", "88vw", "0vw"]
  );
  const displayContainerOpacity = useTransform(
    scrollYProgress,
  ...
  const beamLeftEdge = useTransform(
    ...
    [
      ...
      0.721, // 75px (absolútny hard skok pri prvom dotyku nárazníka)
      0.723, // 125px (nový bod - krivka kapoty)
      0.725, // 158px (nový bod - plynulý nábeh)
      0.728, // 201px (plynulo prechádza po kapote)
      0.737, // 220px (nový bod)
      0.755, // 233px (plynulo po predné sklo)
      0.783, // 233px (drží rovno po dverách k zrkadlu)
      0.785, // 250px (plynulo vybehne na zrkadlo)
      0.7879, // 250px (drží na špičke zrkadla)
      0.788, // 232px (absolútny hard skok dole za zrkadlom)
      0.847, // 232px (drží rovno pozdĺž zadných dverí)
      0.863, // 222px (nový bod - klesanie za dverami)
      0.874, // 202px (plynulo klesá po zadnom kufri)
      0.8789, // 100px (koniec auta / zadný nárazník)
      0.879, // 0px (nekonečno - absolútny hard skok späť)
    ],
    [
      "0px",
      "0px",
  ...
  const carY = useTransform(
    scrollYProgress,
    [TIMING.carStart, TIMING.carCenter, TIMING.carEnd],
    ["100vh", "0vh", "-100vh"]
  );
  ...
  const displayBgColor = useTransform(
    scrollYProgress,
    [0.0, 0.7233, 0.7234, 0.8789, 0.879],
        ...
        <motion.div style={{ opacity: sceneOpacity }} className="relative w-full h-full flex items-center justify-center">
          ...
          <div className="absolute top-32 left-0 right-0 flex flex-col items-center justify-start px-6 text-center z-50 pointer-events-none">
            <motion.h2
              ...
              className="absolute text-2xl md:text-4xl text-neutral-900 dark:text-white font-bold max-w-xl"
            >
              Srdce projektu. <br />
              <span className="text-blue-500">Miniatúrny ToF senzor.</span>
            </motion.h2>
            <motion.h2
            ...
            >
              Plne skrytý. <br />
              <span className="text-neutral-500 dark:text-neutral-400">Zasunutý do trubky riadidiel.</span>
            </motion.h2>
            <motion.h2
              style={{ opacity: textFitOpacity }}
              className="absolute text-2xl md:text-4xl text-neutral-900 dark:text-white font-bold max-w-2xl"
            >
              Štandardný rozmer. <br />
              <span className="text-neutral-500 dark:text-neutral-400">Pasuje na všetky rovné riadidlá.</span>
            </motion.h2>
            <motion.h2
              style={{ opacity: text3Opacity }}
              className="absolute text-2xl md:text-4xl text-neutral-900 dark:text-white font-bold max-w-xl"
            >
              Nekompromisný lúč. <br />
              <span className="text-red-500">Meria 50x za sekundu.</span>
            </motion.h2>
            <motion.div
              style={{ opacity: dbNotificationOpacity }}
              className="absolute mt-10 bg-red-100 dark:bg-red-900/30 border border-red-500 text-red-700 dark:text-red-300 px-6 py-3 rounded-[10px] font-mono text-sm backdrop-blur-md shadow-sm"
            >
          ...
          {/* --- 1. SENZOR --- */}
          <motion.div
            style={{ opacity: sensorOpacity, scale: sensorScale }}
          ...
          {/* --- 2.5. TOP VIEW BICYKLA --- */}
          <motion.div
            style={{
              y: monopoleY,
              opacity: monopoleOpacity,
              top: "calc(50% - 480px)",
              left: "calc(50% + 380px)",
            }}
          ...
          >
            <img
              src={monopoleSrc}
              alt="Top view bicykla"
              className="w-[975px] md:w-[1219px] lg:w-[1381px] max-w-none dark:invert opacity-80"
              style={{
          ...
          <div className="absolute z-10 top-1/2 left-0 right-[calc(50%+48px)] h-[2px] -translate-y-1/2 flex items-center overflow-visible pointer-events-none">
            <motion.div style={{ left: beamLeftEdge, right: 0, opacity: beamOpacity }} className="absolute h-full bg-red-500 shadow-sm" />
          </div>
          ...
          >
            <img src={carSrc} alt="Auto" className="w-full h-auto dark:invert opacity-90 transition-all duration-300" />
          </motion.div>
        </motion.div>
      </div>
    </section>
